...
def parse_text_to_json(text):
    ...
    # Sau khi xử lý các dòng, tổng hợp nội dung:
    for chapter in data["chapters"]:
        for article in chapter["articles"]:
            combined_article = article["raw_content"]
            for clause in article["clauses"]:
                combined_article += "\n" + clause["raw_content"]
                for sub in clause["sub_clauses"]:
                    combined_article += "\n" + sub["raw_content"]
            article["raw_content"] = combined_article.strip()
            article_dict[article["article_number"]] = article["raw_content"]
        combined_chapter = chapter["raw_content"]
        for article in chapter["articles"]:
            combined_chapter += "\n" + article["raw_content"]
        chapter["raw_content"] = combined_chapter.strip()
        chapter_dict[chapter["chapter_number"]] = chapter["raw_content"]
